        ...
        >
          ...
          <div class="overflow-auto">
            <ul class="divide-y divide-gray-100">
              <div v-for="(orderDrop, indexOrderDrop) in results" :key="indexOrderDrop">
                <li
                  v-for="(item, index) in orderDrop.order_items"
                  :key="index"
                  :ref="(el) => { resultsRefs[index] = el;}"
                  :class="selectedIndex === index ? 'bg-gray-100' : ''"
                  @mousemove="selectedIndex = index"
                  class="flex items-center justify-between px-4 py-2.5 relative"
                >
                  <div class="flex">
                    <img
                    ...
                    <button
                      @click="
                        verifyBarcodeIncreaseDropQuantity(orderDrop.id, item.goods_id)
                      "
                    >
                  ...
                  <div class="text-sm text-gray-400">
                    ...
                    <div>
                      Total Price:
                      <span class="text-gray-600 font-semibold"
                        >{{ item.currency }}{{ item.total_price }}</span
                      >
                    </div>
                  </div>
                </li>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const emit = defineEmits(["reloadBasket"]);

    const verifyBarcodeIncreaseDropQuantity = (order_drop_id, product_id) => {
      axios
        .put(route("baskets.users.confirm.barcode", order_drop_id), {
          product_id: product_id,
        })
        .then((response) => {
          if (response.data.errors) {
                addToast(response.data.errors);
          } else {
            isOpen.value = false;
            addToast(response.data.success);
            ctx.emit('reloadBasket', 10);
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    return {
      ...
      isOpen,
      keyboardShortcut,
      verifyBarcodeIncreaseDropQuantity,
      addToast,
    };
  },
};
