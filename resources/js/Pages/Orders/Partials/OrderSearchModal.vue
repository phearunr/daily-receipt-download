<template>
    <button
      ...
      <span class="flex-none text-xs font-semibold text-gray-400">{{ keyboardShortcut }}</span>
    </button>
          ...
          >
            ...
            <div class="overflow-auto">
              ...
              >
                ...
                >
                <div class="flex">
                  <img :src="item.featured_image" :alt="item.buyer_name"
                       class="w-16 h-16 rounded-full object-cover border-white border-2 shrink-0 bg-gray-200"
                  >
                  <a :href="route('stores.orders.index', { term: item.order_id })">
                  ...
                  <div class="text-sm text-gray-400">
                    <div>
                        Total Quantity:
                        <span class="text-gray-600 font-semibold">{{ item.total_quantity}}</span>
                    </div>
                    <div>
                        Grand Total:
                        <span class="text-gray-600 font-semibold">{{item.currency}}{{ item.grand_total_price}}</span>
                    </div>
                  </div>
                </li>
              </ul>
  ...
  export default {
    ...
    setup() {
      ...
      const search = debounce(async (term) => {
        let { data } = await axios.get(route('stores.orders.index'), {params: {term: term}});
